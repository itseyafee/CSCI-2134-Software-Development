...
public class TrinaryHeap {
  ...
  public TrinaryHeap(ArrayList<Integer> priorities, ArrayList<Integer> items) {
    assert priorities != null: "its a null value";
    assert items != null: "its a null value";
  ...
  public void add(int priority, int item) {
    assert nodes != null: "its a null value";
  ...
  public void addException(int priority, int item){
    if (nodes == null){
      throw new NullPointerException("Is not initialized");
    }
    if (size >= nodes.length){
      throw new IndexOutOfBoundsException("new node not added");
    }
  ...
  public void extendNodes(int extend) {
    ...
    System.arraycopy(prevNodes, 0, nodes, 0, prevNodes.length);
  }
  ...
  public boolean containsHelper(int item, int index) {
    ...
    return containsHelper(item, child1) ||
  ...
  public Scanner toScanner() {
    ...
    for (int i = 0; i < size; i++) {
      s.append(nodes[i].item).append(" ");
    }
    return new Scanner(s.toString());
  }
}
